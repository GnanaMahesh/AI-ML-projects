...
class History:
    ...
    def get_valid_actions(self):
        ...
        for i in range(9*self.num_boards):
            if self.boards[i // 9][i % 9] == '0' and active_board_stats[i//9] == 1:
                result.append(i)
        return result
...
def maxmin(history_obj, max_player_flag):
    ...
    if max_player_flag:
        ...
        for action in history_obj.get_valid_actions():
            ...
            if new_history_str not in board_positions_val_dict:
                board_positions_val_dict[new_history_str] = maxmin(new_history, False)
            value = max(value, board_positions_val_dict[new_history_str])
        board_positions_val_dict[new_history_str] = value
        return value
    else:
        value = math.inf
        for action in history_obj.get_valid_actions():
            ...
            value = min(value, board_positions_val_dict[new_history_str])
        board_positions_val_dict[new_history_str] = value
        return value
    # TODO implement
