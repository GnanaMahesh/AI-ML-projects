...
def w_closed_form(X, Y):
    ...
    w_closed= torch.round( w_closed * 10000)/10000
...
def transform_features(X, degree=1):
    ...
    n = X.shape[0]
    phi_X = torch.zeros((n, degree+1), dtype=torch.float64)
    for i in range(n):
      for j in range(degree+1):
            phi_X[i][j] = torch.pow(X[i],j)
    # Student code end
    return phi_X
...
def l2_loss(X, Y, w):
    ...
    # loss = YOUR CODE HERE
    Y = Y - torch.matmul(X,w)
    n,m = Y.size()
    loss = torch.matmul(Y.t(),Y)/n
    loss = loss[0][0]
    # Student code end
...
def l2_loss_derivative(X, Y, w):
    ...
    n,m = Y.shape
    derivative= 2*(torch.matmul(torch.matmul(torch.t(X),X),w) - torch.matmul(torch.t(X),Y))/n
    # Student code end
