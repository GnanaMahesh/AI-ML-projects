...
class RNN:
    ...
    def forward(self, X, a_prev):
        ...
        Parameters:
        X (ndarray): Input data of shape (seq_length, 1){change to onehot encoding of vocabsize}
    ...
    def backward(self,x, a, y_preds, targets):
        ...
        for i in reversed(range(len(x))):
            lby = np.array(y_preds[i])
            lby[targets[i]] -= 1
            # Calculate derivative of output probability vector
            lba= np.dot(self.Wya.T,lby)+da_next
    ...
    def loss(self, y_preds, targets):
        ...
        ypredsarray = np.stack([y_preds[i].flatten() for i in range(len(y_preds))], axis=0)
        ypredsarray = -np.log(ypredsarray)
        loss = np.sum(ypredsarray[np.arange(len(ypredsarray)), targets.reshape(-1,)])
        ### STUDENT CODE ENDS HERE
