...
def gradient_descent(X, y, eta, max_steps):
    ...
    while step<max_steps :    
      n,m = y.shape
      derivative= 2*(np.dot(np.dot(np.transpose(X),X),w) - np.dot(np.transpose(X),y))/n
      w = w - eta * derivative
      w_values.append(w)
      step = step +1
    ### YOUR CODE ENDS HERE ###
...
def stochastic_gradient_descent(X, y, eta, max_steps):
    ...
    step =0
    while step<max_steps : 
      x = np.reshape(X[step],(1,-1))
      g =  np.reshape(y[step],(1,-1)) 
      derivative= 2*(np.dot(np.dot(np.transpose(x),x),w) -np.dot(np.transpose(x), g))
      w = w - eta * derivative
      w_values.append(w)
      step = step +1
    ### YOUR CODE ENDS HERE ###
