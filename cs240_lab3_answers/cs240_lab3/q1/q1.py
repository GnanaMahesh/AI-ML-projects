...
def create_weights(data_dictionary, lambda_val):
    ...
    X = np.array(data_dictionary["X_train"])
    Y = np.array(data_dictionary["Y_train"])
    m,n = np.shape(X)
    i_mat = np.identity(n)
    weights = np.dot(np.dot(np.linalg.inv(np.dot(np.transpose(X),X) + lambda_val * i_mat),np.transpose(X)),Y)
...
def generate_test_error(data_dictionary, weights):
    ...
    X = np.array(data_dictionary["X_test"])
    Y = np.array(data_dictionary["Y_test"])
    Y= np.dot(X,weights) - Y
    n,m = X.shape
    req = np.dot(np.transpose(Y),Y)
    test_error= req[0][0]/n
...
def find_optimal_lambda(error_array, lambda_array):
    ...
    need = np.argmin(error_array)
    optimal_lambda = lambda_array[need]
