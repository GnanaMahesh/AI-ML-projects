...
def lasso_loss(X, Y, w, lambd):
    ...
    L = Y - torch.matmul(X,w)
    n,m = L.size()
    loss = torch.matmul(L.t(),L)/n
    loss = loss[0][0] + lambd * (torch.sum(torch.abs(w)))
    ### YOUR CODE ENDS HERE ###
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    ### YOUR CODE BEGINS HERE ###
    n,m = Y.shape
    derivative= 2*(torch.matmul(torch.matmul(torch.t(X),X),w) - torch.matmul(torch.t(X),Y))/n + lambd*torch.sign(w)
    ### YOUR CODE ENDS HERE ###
